...
class TestHaloUpdater( unittest.TestCase ):
    ...
    def test_save_and_load_mtree_halo_files( self ):
        ...
        # Load
        new_halo_data = ahf_updater.HaloUpdater( data_dir )
        new_halo_data.get_mtree_halos( index='snum', tag=save_tag )
        actual = new_halo_data.mtree_halos[10]['ID']
        actual_detail = new_halo_data.mtree_halos[0]['ID'][500]
...
class TestMassRadii( unittest.TestCase ):
    ...
    @patch( 'galaxy_dive.galaxy_linker.linker.GalaxyLinker.get_mass_radius', )
    @patch( 'galaxy_dive.galaxy_linker.linker.GalaxyLinker.__init__', )
...
class TestEnclosedMass( unittest.TestCase ):
    ...
    def test_get_enclosed_mass( self, mock_init, mock_redshift, mock_init_finder, ):
        ...
        with patch.object( particle_data.ParticleData, 'data', new_callable=PropertyMock, create=True ) as mock_data, \
            ...
            patch.object( gal_linker.GalaxyLinker, 'mass_inside_all_halos', new_callable=PropertyMock, \
    ...
    @patch( 'galaxy_dive.galaxy_linker.linker.GalaxyLinker.find_containing_halos', )
    @patch( 'galaxy_dive.galaxy_linker.linker.GalaxyLinker.__init__', )
    @patch( 'galaxy_dive.analyze_data.particle_data.ParticleData.__init__', )
...
class TestAverageInsideGalaxy( unittest.TestCase ):
    ...
    @patch( 'galaxy_dive.galaxy_linker.linker.GalaxyLinker.weighted_summed_quantity_inside_galaxy', )
    @patch( 'galaxy_dive.galaxy_linker.linker.GalaxyLinker.__init__', )
