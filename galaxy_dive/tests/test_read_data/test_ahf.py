...
class TestAHFReader( unittest.TestCase ):
    ...
    def test_get_profiles( self ):
        ...
        self.ahf_reader.get_profiles( 600 )

        # Halo mass at z=0 for mtree_halo_id = 0
        expected = 0.3149
        actual = self.ahf_reader.profiles['r'][16]
        npt.assert_allclose( expected, actual )

        # ID at an early snapshot (snap 10) for halo file 0
        expected = 2047382.
        actual = self.ahf_reader.profiles['M_in_r'][44]
        npt.assert_allclose( expected, actual )

        expected = np.array([ 0, 44, 82, 119, 155 ])
        actual = self.ahf_reader.profile_id_mapping
        npt.assert_allclose( expected, actual )
