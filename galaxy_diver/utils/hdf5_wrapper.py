...
class HDF5Wrapper(object):
  ...
  def insert_data(self, new_data, index_key=None):
    '''Insert data. Does this by making whole new copy, appending to it, sorting that by the index, and saving the sorted version.

    Args:
      new_data (dict): The new data to be sorted.
      index_key (str): What key to sort the data by. Defaults to None, which will use the key inherent in the data set, and fail otherwise.
    '''
